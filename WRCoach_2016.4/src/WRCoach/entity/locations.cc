...
Locations::Locations(WRTeam *team) {
    _team = team;
}
...
bool Locations::_isInsideArea(const Position &pos, float factor, const Position &goal, const Position &goalLeftMidPost, const Position &goalRightMidPost) {
    if(GlobalConstants::competition() == Competitions::SSL) {
        ...
        // Circular area
        Position midArea = WR::Utils::threePoints(goal, this->fieldCenter(), fieldDefenseLength()/2, 0.0);
        if(WR::Utils::distance(midArea, pos) <= VSS_GOAL_ARCRADIUS*factor)
            return true;

        // Rectangular area
        if(fabs(pos.y()) < (fieldDefenseWidth()/2.0)*factor) {
            if(goal.x()>0.0f) {
                if(pos.x() > (goal.x()-(fieldDefenseLength()*factor)))
                    return true;
            } else {
                if(pos.x() < (goal.x()+(fieldDefenseLength()*factor)))
                    return true;
