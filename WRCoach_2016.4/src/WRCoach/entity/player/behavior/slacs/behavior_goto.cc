...
QString Behavior_GoTo::name() {
    return "Behavior_GoTo";
}

Behavior_GoTo::Behavior_GoTo() {
    setDestination(Position(true, 0.0, 0.0, 0.0));
    setPositionToLook(Position(true, 0.0, 0.0, 0.0));

	_sk_goTo = NULL;
}

void Behavior_GoTo::configure() {
    // Uses
	usesSkill(_sk_goTo = new Skill_GoTo());
}

void Behavior_GoTo::run() {
    _sk_goTo->setDestination(_destination);
    _sk_goTo->setPositionToLook(_posToLook);
