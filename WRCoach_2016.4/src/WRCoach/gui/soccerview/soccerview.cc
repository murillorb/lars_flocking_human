...
GLSoccerView::GLSoccerView(const Competitions::Competition &fieldType, QWidget* parent) :
...
void GLSoccerView::drawRobot(int team, bool hasAngle, bool useDisplayLists)
{
  ...
  if(_fieldType == Competitions::VSS) {
  ...
  if(hasAngle) {
	if(_fieldType == Competitions::VSS) {
      if(team==teamYellow)
        glColor3d(0.7137, 0.5490, 0.0);
      else
        glColor3d(0.2235, 0.1843, 0.4627);
      drawQuad(50.0*cos(theta1)-10, 60.0*sin(theta1), 52.0*cos(theta2), 60.0*sin(theta2), RobotZ+0.01);
    } else
...
void GLSoccerView::drawVector(vector2d v1, vector2d v2, double z) {
    ...
    // Triangle
	const double triangle_thickness = (_fieldType == Competitions::VSS ? 20 : 50);
