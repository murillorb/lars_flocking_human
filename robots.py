# -*- coding: utf-8 -*-
"""
Created on Tue Oct 31 13:29:24 2017

 -*- coding: utf-8 -*-


 
@author: murillo, arsart and Buu
"""
import numpy as np
...
class ParticleBox:
    ...
    def step(self, dt):
        ...
        Ur = np.zeros((self.state.shape[0], 2), dtype=float)#repulsion
        Uo = np.zeros((self.state.shape[0], 2), dtype=float)#orientation
        Ua = np.zeros((self.state.shape[0], 2), dtype=float)#avoidance
        # find pairs of particles undergoing a collision
        D = squareform(pdist(self.state[:, 0:2]))#calculate the distance between all the particles
        #Calculate U
